...
class Runner:
    ...
    def __init__(self, runner_iri: str):
        ...
        # Hotfix: serialize writes to the gRPC normal stream.
        # gRPC does not allow concurrent writes on the same stream.
        self._normal_stream_write_lock = asyncio.Lock()
        self._normal_stream_closed = False
    ...
    async def connect(self, stub: service_pb2_grpc.RunnerStub):
        ...
        # IMPORTANT:
        # gRPC aio streams do not allow concurrent write operations.
        # ReaderInstance and WriterInstance can both call self._write from different tasks,
        # so we serialize all writes through this lock.
        async def writable(msg: service_pb2.FromRunner):
            async with self._normal_stream_write_lock:
                if self._normal_stream_closed:
                    self.logger.warning(
                        "Tried to write to normal stream after it was already closed."
                    )
                    return
                ...
                try:
                    ...
                    self._normal_stream_closed = True
                    raise
    ...
    async def run(self, grpc_url: str):
        ...
        async with grpc.aio.insecure_channel(grpc_url) as channel:
            ...
            async with self._normal_stream_write_lock:
                if not self._normal_stream_closed:
                    try:
